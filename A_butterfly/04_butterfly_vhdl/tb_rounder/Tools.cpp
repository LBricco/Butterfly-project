...
unsigned int Tools::isInt(string number) {
    ...
    for (int i = 0; i < strlen(c_number); i++) { // scorro un carattere per volta
        if ( c_number[i] < 48 || c_number[i] > 57 ) { // cifre decimali --> codici ASCII 48-57
            correct = 0;
...
unsigned int Tools::isBin(string number) {
    ...
    for (int i = 0; i < strlen(c_number); i++) { // scorro un carattere per volta
        if ( c_number[i] != 48 && c_number[i] != 49 ) { // cifre binarie --> codici ASCII 48-49
            correct = 0;
            continue; // esci dal for
        }
    }
...
unsigned int Tools::isHex(string number) {
    ...
    for (int i = 0; i < strlen(c_number); i++) { // scorro un carattere per volta
        if ( ! ( ( c_number[i] >= 48 && c_number[i] <= 57 ) || // cifre da 0 a 9 --> codici ASCII 48-57
                    ...
                    ( c_number[i] >= 97 && c_number[i] <= 102 ) ) ) { // lettere minuscole a-f --> codici ASCII 97-102
            correct = 0;
            continue;
        }
    }
